      ...
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <el-button @click="toEditProductList(scope.$index, scope.row)" type="primary" size="small" icon="el-icon-edit" >编辑</el-button>
            <el-button type="danger" size="small" icon="el-icon-delete" @click="delProductList(scope.$index, scope.row)">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
    </div>
...
export default {
  ...
  methods:{
    ...
    delProductList(index,row){
      ...
      this.$axios.get('/admin/product/delProductList?ID=' + id).then(res=>{
        if(res.data==='yes'){
          console.log(666)
        }
      })
