<template>
  <div id="user">
    <div class="top">
        <span>登入名</span>
        <el-input placeholder="请输入" v-model='checkInfo.username' style='width:15%' class='topRight'></el-input>
        <span>手机</span>
        <el-input placeholder="请输入" v-model='checkInfo.userTel' style='width:15%' class='topRight'></el-input>
        <span>邮箱</span>
        <el-input placeholder="请输入" v-model='checkInfo.email' style='width:15%' class='topRight'></el-input>
        <span>角色</span>
    ...
    <div class='listTable' style='padding:0 20px' >
        <el-table
            ...
            <el-table-column
              ...
              label="操作">
              <template slot-scope="scope">
                <el-button type="primary" size="small" icon="el-icon-edit" @click="editUser(scope.$index, scope.row)">编辑</el-button>
                <el-button type="warning" size="small" icon="el-icon-delete" @click="delUser(scope.$index, scope.row)">删除</el-button>
              </template>
            </el-table-column>
          </el-table>
    </div>
...
export default {
  ...
  methods:{
    ...
    removeUser(){
      if(confirm("您确定要删除？")){
        let id = this.Id;
        this.$axios.post('/admin/user/usersDelete',{ID:id}).then(res=>{
    ...
    btnSearch(){
      ...
      this.$axios.post('/admin/user/checkResult',{
