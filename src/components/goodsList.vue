<template>
  <div id="goodsList">
    <div class="top">
        <span>关键字</span>
        <el-input placeholder="请输入商品名称" v-model="wordSpace" @input='byNameGetGoodsList' style='width:217px' class='topRight'></el-input>
        <span>商品分类</span>
        <el-select v-model="select" slot="prepend" placeholder="请选择分类" >
          <el-option :label="item.list" v-for="(item,key) in copy" :key="key" :value="item.list">{{item.list}}</el-option>
    ...
    <div class='listTable' style='padding:0 20px' >
        <el-table
          ...
          <el-table-column
            ...
            width="200">
            <template slot-scope="scope">
              <el-button @click="editGoodsList(scope.row)" type="primary" size="small" icon="el-icon-edit">编辑</el-button>
              <el-button type="warning" size="small" icon="el-icon-delete" @click="delGoodsList(scope.row)">删除</el-button>
            </template>
          </el-table-column>
        </el-table>
        <div class="block">
...
export default {
  ...
  methods: {
    ...
    //通过关键字获取列表数据
    byNameGetGoodsList(){
      this.$axios.get('/admin/product/byNameGetGoodsList?name='+this.wordSpace
      ).then(res=>{
    ...
    delGoodsList(row){
      if(confirm("您确定要删除？")){
        ...
        this.$axios.get(`/admin/product/delGoodsList?ID=${ID}`).then(res=>{
          if(res.data==='yes'){
            console.log(666)
          }
        })
    ...
    //删除多个商品列表信息
    removeProductList(){
      this.$axios.post('/admin/product/removeProductList',{'ID':this.Id}).then(res=>{
          console.log(res.data)
          if(res.data=='yes'){
            this.getProductList();
            this.$message.success('删除成功')
